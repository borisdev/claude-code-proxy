...
# Get reasoning effort configuration for o3 models
O3_HAIKU_EFFORT = os.environ.get(
    "O3_HAIKU_EFFORT", "medium"
)  # Default effort for haiku mapping
O3_SONNET_EFFORT = os.environ.get(
    "O3_SONNET_EFFORT", "high"
)  # Default effort for sonnet mapping
...
def parse_model_config(model_env_var, default_model):
    """Parse model configuration which can be either a string or JSON."""
    model_str = os.environ.get(model_env_var, default_model)
    try:
        ...
        if isinstance(model_config, dict) and "model" in model_config:
            ...
            # If JSON but not in expected format, treat as string
            return {"model": str(model_config)}
    except (json.JSONDecodeError, ValueError):
        # Not JSON, treat as simple string
        return {"model": model_str}

BIG_MODEL_CONFIG = parse_model_config("BIG_MODEL", "gpt-4.1")
SMALL_MODEL_CONFIG = parse_model_config("SMALL_MODEL", "gpt-4.1-mini")

# Extract model names for backward compatibility
BIG_MODEL = BIG_MODEL_CONFIG["model"]
SMALL_MODEL = SMALL_MODEL_CONFIG["model"]
...
def convert_anthropic_to_litellm(anthropic_request: MessagesRequest) -> Dict[str, Any]:
    ...
    if anthropic_request.model and "o3" in anthropic_request.model.lower():
        # Determine effort level based on the original request model
        original_model = getattr(
            anthropic_request, "original_model", anthropic_request.model or ""
        )

        reasoning_effort = None
        
        if original_model and "haiku" in original_model.lower():
            # Haiku mapping → use effort from SMALL_MODEL_CONFIG or fallback to env var or default
            reasoning_effort = SMALL_MODEL_CONFIG.get("reasoning_effort") or O3_HAIKU_EFFORT
            logger.debug(
                f"Using reasoning_effort={reasoning_effort} for haiku → o3 mapping (from SMALL_MODEL_CONFIG)"
            )
        elif original_model and "sonnet" in original_model.lower():
            # Sonnet mapping → use effort from BIG_MODEL_CONFIG or fallback to env var or default
            reasoning_effort = BIG_MODEL_CONFIG.get("reasoning_effort") or O3_SONNET_EFFORT
            logger.debug(
                f"Using reasoning_effort={reasoning_effort} for sonnet → o3 mapping (from BIG_MODEL_CONFIG)"
            )
        else:
            # Direct o3 model call → check if the model matches BIG or SMALL config
            if anthropic_request.model and anthropic_request.model.endswith(f"/{BIG_MODEL}"):
                reasoning_effort = BIG_MODEL_CONFIG.get("reasoning_effort", "high")
                logger.debug(
                    f"Using reasoning_effort={reasoning_effort} for direct {BIG_MODEL} call"
                )
            elif anthropic_request.model and anthropic_request.model.endswith(f"/{SMALL_MODEL}"):
                reasoning_effort = SMALL_MODEL_CONFIG.get("reasoning_effort", "medium")
                logger.debug(
                    f"Using reasoning_effort={reasoning_effort} for direct {SMALL_MODEL} call"
                )
            else:
                # Fallback to high effort
                reasoning_effort = "high"
                logger.debug(
                    f"Using reasoning_effort=high (default) for direct o3 model: {anthropic_request.model or 'None'}"
                )
        
        if reasoning_effort:
            litellm_request["reasoning_effort"] = reasoning_effort
            logger.info(
                f"🧠 O3 MODEL: {anthropic_request.model} with reasoning_effort={reasoning_effort}"
            )
...
async def create_message(request: MessagesRequest, raw_request: Request):
    try:
        ...
        # Log all error details (convert non-serializable objects to strings)
        ...
        logger.error(
            f"Error processing request: {json.dumps(serializable_details, indent=2)}"
        )
